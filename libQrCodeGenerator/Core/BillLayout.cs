...
using System.Text;
using static libQrCodeGenerator.SwissQRBill.Generator.Address;
...
namespace libQrCodeGenerator.SwissQRBill.Generator
{
    ...
    internal class BillLayout
    {
        ...
        private void DrawScissorsBlade(double x, double y, double size, double angle, bool mirrored)
        {
            ...
            matrix.Scale(mirrored ? -scale : scale, scale);
            _graphics.SetTransformation(matrix.TranslationX, matrix.TranslationY, angle, mirrored ? -scale : scale, scale);
