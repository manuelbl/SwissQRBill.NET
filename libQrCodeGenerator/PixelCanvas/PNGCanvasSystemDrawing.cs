...
namespace libQrCodeGenerator.SwissQRBill.PixelCanvas
{
        ...
        : AbstractCanvas
    {
        ...
        // SwissQRBill.NET\Examples\WindowsForms\SystemDrawing\SystemDrawingCanvas.cs
        ...
        protected PNGCanvas(string fontFamilyList)
        {
            // setup font metrics
        ...
        public PNGCanvas(double width, double height, float dpi, string fontFamilyList)
            : this(fontFamilyList)
        {
            double xOffset = width / 25.4 * dpi;
            double yOffset = height / 25.4 * dpi;

            this._bmp = new System.Drawing.Bitmap((int)xOffset, (int)yOffset, System.Drawing.Imaging.PixelFormat.Format24bppRgb);
            this._bmp.SetResolution(dpi, dpi);
            Graphics graphics = System.Drawing.Graphics.FromImage(this._bmp);
            graphics.FillRectangle(Brushes.White, new Rectangle(0, 0, _bmp.Width, _bmp.Height));
            // graphics.FillRectangle(Brushes.HotPink, new Rectangle(0, 0, _bmp.Width, _bmp.Height));

            float scale = dpi / 25.4f;

            // SetOffset((float)xOffset, (float)yOffset);
            SetOffset(0f, (float)yOffset);
            InitGraphics(graphics, true, scale);
        }


        // public PNGCanvas(Graphics graphics, double xOffset, double yOffset, float scale, string fontFamilyList)
        //: this(fontFamilyList)
        // {
        //     SetOffset((float)xOffset, (float)yOffset);
        ...
        public override byte[] ToByteArray()
        {
            ...
            {
                this._bmp.Save(stream, System.Drawing.Imaging.ImageFormat.Png);
